...
public:
  explicit VoronoiD3(const ActionOptions&);
  ~VoronoiD3();
// active methods:
...
PLUMED_REGISTER_ACTION(VoronoiD3,"VORONOID3")

void VoronoiD3::registerKeywords( Keywords& keys ) {
...
VoronoiD3::VoronoiD3(const ActionOptions&ao):
...
void VoronoiD3::calculate()
{
  ...
  #pragma omp parallel num_threads(nt)
  {
    ...
    for(unsigned int j=0; j<num_atomsa; j+=1) {
      if(j==num_atomso)     charge[j]-=d1;       //N-1   equals the charge of N in NNH (if only one N is used, D_2 and D_3 are not needed to set in input file)
      else if(j==num_atomso+1) charge[j]-=d2;    //N-1   equals the charge of N in NNH (if you want use second N)
      else if(j==num_atomso+2) charge[j]-=d3;    //N-1   equals the charge of N in NNH (if you want use third  N)
    ...
    for(unsigned int j=0;j<num_atomsa;j++) {
      for(unsigned int k=j+1;k<num_atomsa;k++) {
		...
		//only consider distance between [O in water] and [N in NNH]
		if (j<num_atomso && k>num_atomso-1) {
        ...
        IonDistance -= buffer; //this is the IonDistance for NNH, the result!
    ...
    for(unsigned int i=0; i<nn; i+=1) {  //it is equivalent to the loop on n,j of Emanuele. The derivative will be evaulated on these two atoms
        ...
        for(unsigned int k=0;k<num_atomsa;k++) {
          ...
          for(unsigned int j=k+1;j<num_atomsa;j++) {
			...
			//if(j==k) distGA[j][k] = 0.0;

			if (k<num_atomso && j>num_atomso-1) {
