...
# compulsory
parser.add_argument('--sigma','-s',dest='sigma',type=str,default="0.02,0.02",help='the bandwidth for the kernel density estimation. Use e.g. the last value of sigma from an OPES_METAD simulation')
...
kbt_group.add_argument('--temp',dest='temp',type=float,default='300',help='the temperature in Kelvin. Energy units is Kj/mol')
# input columns
parser.add_argument('--cv',dest='cv',type=str,default='2,3',help='the CVs to be used. Either by name or by column number, starting from 1')
...
# grid related
parser.add_argument('--min',dest='grid_min',type=str,default="-1.2,-0.5",help='lower bounds for the grid')
parser.add_argument('--max',dest='grid_max',type=str,default="1.2,13.5", help='upper bounds for the grid')
parser.add_argument('--bin',dest='grid_bin',type=str,default="500,500", help='number of bins for the grid')
# blocks
split_group=parser.add_mutually_exclusive_group(required=False)
split_group.add_argument('--blocks',dest='blocks_num',type=int,default=5,help='calculate errors with block average, using this number of blocks')
...
parser.add_argument('--skiprows',dest='skiprows',type=int,default=50000,help='skip this number of initial rows')
