...
if [ ! -f tag_0_finished ] ;then
    { if [ ! -f model.ckpt.index ]; then
        ...
        dp train --mpi-log=master run.json --skip-neighbor-stat --init-frz-model ../043_dplr_water_gly_slab_500w/frozen_model.pb;
    else
        CUDA_VISIBLE_DEVICES=0 horovodrun -np 1
        dp train --mpi-log=master run.json --restart model.ckpt --skip-neighbor-stat; fi }
